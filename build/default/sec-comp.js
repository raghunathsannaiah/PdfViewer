...
import "./node_modules/@polymer/polymer/lib/mixins/gesture-event-listeners.js";
...
class SecComp extends PolymerElement {
  static get properties() {
    console.log('PDFVIEW TEMPLATE');
    return {
      /**
      ...
      */
      PDFJS_workerSrc: {
        type: String,
        value: () => {
          return '../node_modules/@webcomponents/pdfjs-dist/build/pdf.worker.min.js';
        }
      },
  ...
  constructor() {
    ...
    this.PDFJS = pdfjsLib; // The workerSrc property shall be specified.

    pdfjsLib.GlobalWorkerOptions.workerSrc = '//mozilla.github.io/pdf.js/build/pdf.worker.js'; //this.PDFJS = pdfjsDistWebPdfViewer;
  ...
  ready() {
    console.log('ttt sss' + this.tagName); // If you override ready, always call super.ready() first.
  ...
  _srcChanged(newValue, oldValue) {
    ...
    this.PDFJS.getDocument(src).promise.then(pdf => {
      if (src !== this.src) {
        // fake cancel
        return;
      }
  ...
  fitWidth() {
    this._PDF.getPage(this.page).then(page => {
  ...
  fit() {
    ...
    this._PDF.getPage(pageNum).then(page => {
  ...
  _renderView(view, pg) {
    ...
    return new Promise((resolve, reject) => {
      this._PDF.getPage(pg).then(page => {
  ...
  _drawPage(pg) {
    ...
    Promise.all(promises).then(() => {
      ...
      const event = new CustomEvent('pdf-viewer-render', {
        detail: {
          page: this.page
        }
      });
